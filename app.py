...
sender_email = "vanhung2332003@gmail.com"
sender_password = "Nguyenvanhung123456"
...
smtp_server.login(sender_email, "qaug uplc agbc dikl")
...
# Set the desired duration to 10 seconds
desired_duration = 10
...
def generate_frames():
    ...
    while True:
        ...
        if motion_detected:
            if out is None:
                video_count += 1
                video_name = f'C:\\Users\\DELL\\OneDrive\\Desktop\\app\\static\\video\\motion_capture_{video_count}.mp4'
                out = cv2.VideoWriter(video_name, fourcc, 25.0, (frame.shape[1], frame.shape[0]))
                send_email()
            out.write(frame)

            # Check if the desired duration (in seconds) has been reached
...
def get_video_list():
    ...
    for video in os.listdir(video_dir):
        video_url = url_for('static', filename='videos/' + video)
